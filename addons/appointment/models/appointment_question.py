...
class AppointmentQuestion(models.Model):
    ...
    def _check_question_type(self):
        incomplete_questions = self.filtered(lambda question: question.question_type in [
                                             'select', 'radio', 'checkbox'] and not question.answer_ids)
    ...
    def action_view_question_answer_inputs(self):
        ...
        if self.question_type in ['select', 'radio', 'checkbox']:
            action['views'] = [(False, 'pivot'), (False, 'graph'),
                               (False, 'list'), (False, 'form')]
        elif self.question_type in ['char', 'text_box']:
            action['views'] = [
                (False, 'list'), (False, 'form'), (False, 'graph')]
        return action
