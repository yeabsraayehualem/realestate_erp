...
class AppointmentPortal(portal.CustomerPortal):
    ...
    def portal_my_appointments(self, page=1, sortby=None, filterby=None, search=None, search_in='all', groupby='none', **kwargs):
        ...
        order = '%s, %s' % (
            groupby_field, sort_order) if groupby_field else sort_order
        ...
        if groupby_field:
            grouped_appointments = [(g, Event.concat(
                *events)) for g, events in groupbyelem(appointments, itemgetter(groupby_field))]
